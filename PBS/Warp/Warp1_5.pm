...
my ($t0_warp, $warp_load_time, $t0_warp_check) = ([gettimeofday], 0) ;
my ($run_in_warp_mode, $number_of_removed_nodes) = (1, 0) ;

if(-e $warp_file)
	{
	($nodes, $node_names, $global_pbs_config, $warp_dependents,$version, $number_of_nodes_in_the_dependency_tree, $warp_configuration, $distribution_digest)
		= do $warp_file or die ERROR("Warp: couldn't evaluate warp file '$warp_file'\nfile error: $!\ncompilation error: $@") . "\n" ;
	
	($warp_load_time, $t0_warp_check) = (tv_interval($t0_warp, [gettimeofday]), [gettimeofday]) ;
...
if($run_in_warp_mode)
	...
	$number_of_removed_nodes = $nodes_in_warp - scalar(keys %$nodes) ;
	$trigger_log            .= "\n" . $trigger_log_warp ;
	...
	if($pbs_config->{DISPLAY_WARP_TIME} && (!$pbs_config->{QUIET} || $number_of_removed_nodes))
		...
		Say EC "<I>Warp: nodes: $nodes_in_warp, triggered: $node_mismatch, "
			. ( $number_of_removed_nodes ? '<W3>' : '' ). "removed: $number_of_removed_nodes"
...
$pbs_config->{INTERMEDIATE_WARP_WRITE} = 
	sub
	{
	my ($dependency_tree, $inserted_nodes) = @_ ;
	...
	PrintInfo "\e[K" ;
	} unless $pbs_config->{NO_PRE_BUILD_WARP} || $run_in_warp_mode ;

my $GenerateWarpFile = 
	sub
	{
	my ($dependency_tree, $inserted_nodes, $exception) = @_ ;
	
	if($exception)
		{
		if($exception =~ /^BUILD_FAILED/)
			{
			# builder failure
			GenerateWarpFile
				(
		...
		# refresh cache, the synch should only be for changed files
		RunUniquePluginSub($pbs_config, 'ClearWatchedFilesList', $warp_signature) ;
		}
	} ;

$nodes, $number_of_removed_nodes, $GenerateWarpFile
}
...
for my $file (sort {($warp_dependents->{$b}{LEVEL} // 0)  <=> ($warp_dependents->{$a}{LEVEL} // 0)} keys %$warp_configuration)
	...
	if ($pbs_config->{DISPLAY_WARP_CHECKED_NODES_FAIL_ONLY} )
		{
		Say EC "\e[K<W3>Warp: removing <I3>$file<I2>, removed nodes: " . scalar(@nodes_triggered)
			if @nodes_triggered ;
		}
...
for my $node (@$nodes_to_check)
	...
	if('VIRTUAL' ne ($nodes->{$node}{__MD5} // '') && !$nodes->{$node}{__VIRTUAL} )
		...
		if (! -e $nodes->{$node}{__BUILD_NAME})
			{
			$remove_this_node++ ;
			push @reasons, "not found" ;
			}
	...
	if($pbs_config->{DISPLAY_WARP_CHECKED_NODES})
		...
		if ($remove_this_node)
			{
			Say EC "\e[K<W3>Warp: removing <I3>$node<I2> [" . join(' ,', @reasons) . "]" ;
			}
		else
			{
			Say EC "\e[K<I>Warp: OK: $node" unless $pbs_config->{DISPLAY_WARP_CHECKED_NODES_FAIL_ONLY} ;
			}
		}
...
{
my ($warp_file, $targets, $dependency_tree, $inserted_nodes, $pbs_config, $node_names, $warp_message) = @_ ;
$warp_message //='' ;

return unless $pbs_config->{DO_BUILD} ;
...
my $js = $pbs_config->{WARP_HUMAN_FORMAT} ? JSON::XS->new->canonical->pretty(1) : JSON::XS->new ;
...
Say Info sprintf "Warp: time: %0.2f s.", tv_interval($t0_warp_generate, [gettimeofday])
	if $pbs_config->{DISPLAY_WARP_TIME} ;
}
