...
You can give attributes to the nodes, the attribute is declare by following the node name with a colon and a textual attribute (see the example below). When B<PBS> finds such an attribute,
