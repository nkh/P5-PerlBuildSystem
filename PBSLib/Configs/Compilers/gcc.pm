...
=item * GENERATE_COVERAGE        

if set, adds SPECIAL_CFLAGS to generate coverage data for gcov.

=item * CC, CPP, CXX, AR, AS, LD, OBJDUMP    

The commands to use. Each compilter suite configuration is expected to set these.

=item * LDFLAGS                  

Mandatory libraries to include when linking the final executable. LDFLAGS gets added
 onto the end of the link command-line.
...
The flags that appear on the C and C++ compiler commandline, respectively. 
Typically '%WFLAGS %SPECIAL_CFLAGS %OPTIMIZE_CFLAGS' or similar.
...
PBS::Config::AddConfigTo 'BuiltIn',
	...
	# syntax
	CC_SYNTAX       => "%%CC %%CFLAGS %%CDEFINES %%CFLAGS_INCLUDE -o %%FILE_TO_BUILD -c %%C_SOURCE",
	CXX_SYNTAX      => "%%CXX %%CXXFLAGS %%CDEFINES %%CFLAGS_INCLUDE -o %%FILE_TO_BUILD -c %%C_SOURCE",
