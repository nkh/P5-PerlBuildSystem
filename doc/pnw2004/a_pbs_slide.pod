...
You can give attributes to the nodes, the attribute is declare by following the node name with a colon and a textual attribute (see the example below).
...
Trigger is a system to  allow the current build to hook into other dependency trees. This can be used to 
