...
if(0 == system 'tmux -V > /dev/null' )
	...
	if('' ne $options)
		...
		chomp $options ;
		#my $command = "tmux send-keys -- " . ('C-H ' x length($ARGV[1])) . " '$options '" ;
		#my $command = "xdotool key " . ('Backspace ' x length($ARGV[1])) . " ; xdotool type -- '$options '" ;
		#qx"$command" ;
		
		my $command = (chr(8) x length($ARGV[1])) . $options ;
